...
export const loadStatefulSetEpic = (action$, state$, { ajax }) =>
  action$.pipe(
    ...
    mergeMap(({ payload, meta: { url, clusterID, namespaceID } }) =>
      ajax(`${url}`).pipe(
        map((resp) =>
          a.loadStatefulSetSuccess(resp, { clusterID, namespaceID })
        ),
        catchError((error) =>
          of(a.loadStatefulSetFailure(error, { clusterID, namespaceID }))
        )
      )
    )
  );
      ...
      ({ payload, meta: { resolve, reject, url, clusterID, namespaceID } }) =>
        ajax({
          ...
          catchError((error) => {
            reject(error);
            return of(
              a.createStatefulSetFailure(error, { clusterID, namespaceID })
            );
          })
        )
    )
  ...
  action$.pipe(
    ...
    mergeMap(({ payload, meta }) =>
      timer(1000).pipe(
        mapTo(
          push(
            `/clusters/${meta.clusterID}/namespaces/${meta.namespaceID}/statefulSets`
          )
        )
      )
    )
      ...
      ({ payload, meta: { resolve, reject, url, clusterID, namespaceID } }) =>
        ajax({
          ...
          catchError((error) => {
            reject(error);
            return of(
              a.updateStatefulSetFailure(error, { clusterID, namespaceID })
            );
          })
        )
    )
  ...
  action$.pipe(
    ...
    mergeMap(({ payload, meta: { url, clusterID, namespaceID } }) =>
      ajax({
        ...
        map((resp) =>
          a.removeStatefulSetSuccess(resp, {
            id: payload,
            clusterID,
            namespaceID,
          })
        ...
        catchError((error) =>
          of(
            a.removeStatefulSetFailure(error, {
              id: payload,
              clusterID,
              namespaceID,
            })
  ...
  action$.pipe(
    ...
    mergeMap(({ payload, meta: { url, clusterID, namespaceID } }) =>
      ajax({
        ...
        map((resp) =>
          a.scaleStatefulSetSuccess(resp, { clusterID, namespaceID })
        ),
        catchError((error) =>
          of(a.scaleStatefulSetFailure(error, { clusterID, namespaceID }))
        )
      )
    )
  );
